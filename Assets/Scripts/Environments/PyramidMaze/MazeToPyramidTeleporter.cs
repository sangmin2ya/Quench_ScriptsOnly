...
public class MazeToPyramidTeleporter : MonoBehaviour
{
    [SerializeField]
    ...
    IEnumerator FadeOut(GameObject player)
    {
        ...
        TeleportPlayerToPyramid(player);
    }

    private void TeleportPlayerToPyramid(GameObject player)
    {
        Underground.SetActive(true);
        player.transform.position = teleportBlock.transform.position;
